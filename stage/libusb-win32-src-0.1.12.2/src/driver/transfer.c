...
NTSTATUS control_transfer(libusb_device_t *dev,libusb_request *pRequest, unsigned char *pTransferBuffer, int transferBufferSize, int *ret)
{
    ...
    if (!dev->config.value)
    {
        DEBUG_ERROR("control_transfer(): invalid configuration 0");
        return STATUS_INVALID_DEVICE_STATE;
    }
    DEBUG_MESSAGE("control_transfer(): size %d", transferBufferSize);
    DEBUG_MESSAGE("control_transfer(): timeout %d", pRequest->timeout);
    ...
    urb.UrbControlTransfer.TransferBufferLength = transferBufferSize;
    urb.UrbControlTransfer.TransferBuffer = pTransferBuffer;
    urb.UrbControlTransfer.TransferFlags = USBD_DEFAULT_PIPE_TRANSFER;
    if (USB_ENDPOINT_DIRECTION_IN(pRequest->control.requestType))
        urb.UrbControlTransfer.TransferFlags |= USBD_TRANSFER_DIRECTION_IN | USBD_SHORT_TRANSFER_OK;

    memcpy(&urb.UrbControlTransfer.SetupPacket,&pRequest->control,8);

    status = call_usbd(dev, &urb, IOCTL_INTERNAL_USB_SUBMIT_URB, pRequest->timeout);

    if (!NT_SUCCESS(status) || !USBD_SUCCESS(urb.UrbHeader.Status))
    {
        DEBUG_ERROR("control_transfer(): failed: status: 0x%x, urb-status: 0x%x", status, urb.UrbHeader.Status);
        *ret = 0;
    }
    else
    {
