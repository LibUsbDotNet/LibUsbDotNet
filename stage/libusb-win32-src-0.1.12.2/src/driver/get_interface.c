...
NTSTATUS get_interface_original(libusb_device_t *dev,
...
NTSTATUS get_interface(libusb_device_t *dev, int interface, unsigned char *altsetting, int *ret, int timeout)
{
    NTSTATUS status = STATUS_SUCCESS;
    URB urb;
    ...
    urb.UrbControlTransfer.TransferBuffer = altsetting;
    urb.UrbControlTransfer.TransferFlags = USBD_DEFAULT_PIPE_TRANSFER | USBD_TRANSFER_DIRECTION_IN | USBD_SHORT_TRANSFER_OK;

    urb.UrbControlTransfer.SetupPacket[0]=(UCHAR)0x80 | BMREQUEST_TO_INTERFACE;		// RequestType
    urb.UrbControlTransfer.SetupPacket[1]=(UCHAR)USB_REQUEST_GET_INTERFACE;			// Request
	*((USHORT*)&urb.UrbControlTransfer.SetupPacket[2]) = (USHORT)0;					// Value
	*((USHORT*)&urb.UrbControlTransfer.SetupPacket[4]) = (USHORT)interface;			// Index
 	*((USHORT*)&urb.UrbControlTransfer.SetupPacket[6]) = (USHORT)1;					// Length

    status = call_usbd(dev, &urb, IOCTL_INTERNAL_USB_SUBMIT_URB, timeout);
    if (!NT_SUCCESS(status) || !USBD_SUCCESS(urb.UrbHeader.Status))
		return  get_interface_original(dev,interface,altsetting,ret,timeout);
    else
    {
