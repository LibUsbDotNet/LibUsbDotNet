...
NTSTATUS dispatch_ioctl(libusb_device_t *dev, IRP *irp)
{
  ...
  switch(control_code) 
    ...
    case LIBUSB_IOCTL_GET_VERSION:
      ...
      request->version.BcdLibUsbDotNetKernelMod = 61;
    ...
    case LIBUSB_IOCTL_ISOCHRONOUS_WRITE:
     ...
     case LIBUSB_IOCTL_GET_DEVICE_PROPERTY:
        if (!request || output_buffer_length < sizeof(libusb_request))
        {
            DEBUG_ERROR("dispatch_ioctl(), LIBUSB_IOCTL_GET_DEVICE_PROPERTY: invalid output buffer");
            status = STATUS_INVALID_PARAMETER;
            break;
        }
        if (!reg_get_device_property(dev->physical_device_object, request->deviceProperty.propertyId, output_buffer, output_buffer_length, &ret))
            status = STATUS_INVALID_PARAMETER;
        break;
