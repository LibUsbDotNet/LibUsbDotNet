...
NTSTATUS dispatch_ioctl(libusb_device_t *dev, IRP *irp)
{
  ...
  switch(control_code) 
    ...
    case LIBUSB_IOCTL_GET_VERSION:
      ...
      request->version.BcdLibUsbDotNetKernelMod = 38;
    ...
    case LIBUSB_IOCTL_CONTROL_TRANSFER:
        if (USB_ENDPOINT_DIRECTION_IN(request->control.requestType))
        {
            status = control_transfer(dev, request,output_buffer, output_buffer_length, &ret);
        }
        else
        {
            status = control_transfer(
                         ...
                         input_buffer_length - sizeof(libusb_request), &ret);
        }
        break;

     case LIBUSB_IOCTL_GET_REG_PROPERTY:
        if (!request || output_buffer_length < sizeof(libusb_request))
        {
            DEBUG_ERROR("dispatch_ioctl(), LIBUSB_IOCTL_GET_REG_PROPERTY: invalid output buffer");
            status = STATUS_INVALID_PARAMETER;
            break;
        }
        if (!reg_get_property_ex(dev->physical_device_object, request->deviceProperty.propertyId, output_buffer, output_buffer_length, &ret))
            status = STATUS_INVALID_PARAMETER;
        break;
    ...
    case LIBUSB_IOCTL_GET_CUSTOM_REG_PROPERTY:
        ...
        {
            DEBUG_ERROR("dispatch_ioctl(), LIBUSB_IOCTL_GET_CUSTOM_REG_PROPERTY: invalid buffer");
            status = STATUS_INVALID_PARAMETER;
            break;
        }
    ...
    case LIBUSB_IOCTL_SET_CUSTOM_REG_PROPERTY:
        if (!input_buffer || (input_buffer_length < sizeof(libusb_request)))
        {
            DEBUG_ERROR("dispatch_ioctl(), LIBUSB_IOCTL_SET_CUSTOM_REG_PROPERTY: invalid buffer");
            status = STATUS_INVALID_PARAMETER;
            break;
        }
        status=reg_set_custom_property(dev, input_buffer, request->deviceKeySet.keyType, request->deviceKeySet.nameOffset, request->deviceKeySet.valueOffset, request->deviceKeySet.valueLength);
        break;
