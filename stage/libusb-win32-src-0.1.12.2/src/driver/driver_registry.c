...
bool_t reg_get_property_ex(DEVICE_OBJECT *physical_device_object, int property, char *data, int size, int* pReturnBytesCount)
{
    ULONG ret;
    ULONG i;
    if (!physical_device_object || !data || !size || !pReturnBytesCount)
    {
        return FALSE;
    }
    *pReturnBytesCount=0;
    memset(data, 0, size);
    if (NT_SUCCESS(IoGetDeviceProperty(physical_device_object, property, size - 2,data, &ret)))
    {
        *pReturnBytesCount=ret;
        return TRUE;
    }
    return FALSE;
}
...
NTSTATUS reg_get_custom_property(libusb_device_t *dev, char *pBuffer, unsigned int bufferSize, unsigned int nameOffset, int* pReturnBytesCount)
{
    ...
    if (NT_SUCCESS(status))
    {
        ...
        length = sizeof(KEY_VALUE_FULL_INFORMATION) + name.MaximumLength + bufferSize;
        info = ExAllocatePool(NonPagedPool, length);
        if (info)
        {
            memset(info, 0, length);
            ...
            if (NT_SUCCESS(status))
            {
                bufferSize = (info->DataLength > bufferSize) ? bufferSize : info->DataLength;
                memcpy(pBuffer, (((char *)info) + info->DataOffset),bufferSize);
                *pReturnBytesCount=bufferSize;
            }
            ExFreePool(info);
        }
        ZwClose(key);
