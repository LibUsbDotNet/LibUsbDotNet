...
namespace LibUsbDotNet.WinUsb {
    ...
    public class LibusbKRegistry : UsbRegistry {
        ...
        /// <summary>
        /// Gets a list of libusbK device paths for the specified interface guid.
        /// </summary>
        ...
        /// <remarks>
        /// Each device path string in the <paramref name="devicePathList"/> represents a seperate libusbK device (interface).
        /// </remarks>
        ...
        /// <summary>
        /// Gets a list of <see cref="LibusbKRegistry"/> classes for the specified interface guid.
        /// </summary>
        /// <param name="deviceInterfaceGuid">The DeviceInterfaceGUID to search for.</param>
        /// <param name="deviceRegistryList">A list of device paths associated with the <paramref name="deviceInterfaceGuid"/>.</param>
        /// <returns>True of one or more device paths was found.</returns>
        /// <remarks>
        /// Each <see cref="LibusbKRegistry"/> in the <paramref name="deviceRegistryList"/> represents a seperate libusbK device (interface).
        /// </remarks>
        public static bool GetLibusbKRegistryList(Guid deviceInterfaceGuid, out List<LibusbKRegistry> deviceRegistryList) {
            ...
            if (deviceInfo != IntPtr.Zero) {
                while ((SetupApi.SetupDiEnumDeviceInterfaces(deviceInfo, null, ref deviceInterfaceGuid, devicePathIndex, ref interfaceData))) {
                    ...
                    if (bResult) {
                        LibusbKRegistry regInfo = new LibusbKRegistry();

                        SetupApi.getSPDRPProperties(deviceInfo, ref devInfoData, regInfo.mDeviceProperties);

                        // Use the actual libusbK device path for SYMBOLIC_NAME_KEY. This will be used to open the device.
        ...
        /// <summary>
        /// Gets a list of available LibUsbK devices.
        /// </summary>
        public static List<LibusbKRegistry> DeviceList {
            get {
                List<LibusbKRegistry> _deviceList = new List<LibusbKRegistry>();
                SetupApi.EnumClassDevs(null, SetupApi.DICFG.ALLCLASSES | SetupApi.DICFG.PRESENT, LibusbKRegistryCallBack, _deviceList);
                /// Keep only the libusbK devices (where the device Class Guid equals <see cref="UsbRegistry.LIBUSBK_DEVICE_CLASS_GUID"/>)
                List<LibusbKRegistry> deviceList = _deviceList.Where(device => ((string)device.DeviceProperties["ClassGuid"]).Equals(LIBUSBK_DEVICE_CLASS_GUID)).ToList();
        ...
        /// <summary>
        /// Gets a collection of DeviceInterfaceGuids that are associated with this libusbK device.
        /// </summary>
        ...
        public override bool IsAlive {
            get {
                ...
                List<LibusbKRegistry> deviceList = DeviceList;
                foreach (LibusbKRegistry registry in deviceList) {
        ...
        ///<summary>
        /// Gets the interface ID this libusbK device (interface) is associated with.
        ///</summary>
        /// <remarks>This value is parsed out of the <see cref="DeviceID"/> field.</remarks>
        ...
        public override bool Open(out UsbDevice usbDevice) {
            ...
            bool bSuccess = Open(out libusbKDevice);
            if (bSuccess)
                usbDevice = libusbKDevice;
            return bSuccess;
        }
        ...
        public bool Open(out LibusbKDevice usbDevice) {
            ...
            if (LibusbKDevice.Open(SymbolicName, out usbDevice)) {
        ...
        /*
        private static bool LibusbKRegistryCallBack(IntPtr deviceInfoSet,
                                                   int deviceIndex,
                                                   ref SetupApi.SP_DEVINFO_DATA deviceInfoData,
                                                   object classEnumeratorCallbackParam1)
        {

            List<LibusbKRegistry> deviceList = (List<LibusbKRegistry>) classEnumeratorCallbackParam1;
                ...
                foreach (String devInterfaceGuid in devInterfaceGuids)
                {
                    ...
                    if (SetupApi.GetDevicePathList(g, out devicePaths))
                    {
                        foreach (string devicePath in devicePaths)
                        {
                            ...
                            LibusbKRegistry foundRegistry=null;
                            foreach (LibusbKRegistry usbRegistry in deviceList)
                            {
        ...
        */
        private static bool LibusbKRegistryCallBack(IntPtr deviceInfoSet,
            ...
            if (bSuccess) {
                ...
                foreach (String devInterfaceGuid in devInterfaceGuids) {
                    Guid g = new Guid(devInterfaceGuid);
                    List<LibusbKRegistry> tempList;
                    if (GetLibusbKRegistryList(g, out tempList)) {
                        foreach (LibusbKRegistry regInfo in tempList) {
                            // Don't add duplicate devices (with the same device path)
                            LibusbKRegistry foundRegistry = null;
                            foreach (LibusbKRegistry usbRegistry in deviceList) {
                                if (usbRegistry.SymbolicName == regInfo.SymbolicName) {
        ...
        /// <summary>
        /// For LibusbKRegistry objects, this always returns <see cref='SymbolicName' />.
        /// </summary>
