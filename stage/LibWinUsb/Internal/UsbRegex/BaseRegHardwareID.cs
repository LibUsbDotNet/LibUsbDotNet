...
{
    internal class BaseRegHardwareID : Regex
    {
        ...
        private const string PATTERN = "Vid_(?<Vid>[0-9A-Fa-f]{1,4})&Pid_(?<Pid>[0-9A-Fa-f]{1,4})(&|$|#)(Rev_(?<Rev>[0-9A-Fa-f]{1,4})(&|$|#)){0,1}";

        public BaseRegHardwareID() : base(PATTERN, OPTIONS) { }
    }
}