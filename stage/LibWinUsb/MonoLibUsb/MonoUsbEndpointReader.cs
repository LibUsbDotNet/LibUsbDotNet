...
using LibUsbDotNet.MonoLibUsb.Internal;
using MonoLibUsb;
...
namespace LibUsbDotNet.MonoLibUsb
{
    ...
    public class MonoUsbEndpointReader : UsbEndpointReader
    {
        ...
        internal MonoUsbEndpointReader(UsbDevice usbDevice, int readBufferSize, ReadEndpointID readEndpointID)
            : base(usbDevice, readBufferSize, readEndpointID) { }
        ...
        public override bool Reset()
        {
            if (IsDisposed) throw new ObjectDisposedException(GetType().Name);
            Abort();
            int ret = MonoLibUsbApi.libusb_clear_halt((MonoUsbDeviceHandle) Device.Handle, EpNum);
            if (ret < 0)
            {
